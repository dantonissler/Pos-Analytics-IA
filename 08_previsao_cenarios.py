...
# 8) `08_previsao_cenarios.py` — Previsão + Simulação de cenários (Monte Carlo)

```python
# Regressão para previsão + análise de cenários via Monte Carlo.
import numpy as np
import pandas as pd
...
# Dados sintéticos: vendas = 2.5*marketing + 1.2*preco_negativo + ruído
rng = np.random.default_rng(42)
n = 400
marketing = rng.normal(50, 10, n)           # investimento (kR$)
preco = rng.normal(100, 5, n)               # preço unitário
...
# Simulação de cenários (Monte Carlo): variar marketing e preço dentro de faixas
...
# Exemplo de 3 cenários executivos
for nome, (mk,p) in {
    "Conservador": (50, 102),
    "Base": (60, 100),
    "Agressivo": (75, 96)
